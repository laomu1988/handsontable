...
var options = {
    ...
    metas: [{ row: 0, col: 0, meta: { className: 'htRight' } }]
};
...
addListener('#addLine', function () {
    var data = hot.getData();
    var addData = data[0].map(function (v) {
        return v;
    });
    hot.insertRow(0, addData);
});

addListener('#deleteLine', function () {
    hot.deleteRow(0);
});

addListener('#disabled', function () {
...
function addListener(selector, callback) {
    document.querySelector(selector).addEventListener('click', callback, false);
}